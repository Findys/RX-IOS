    ...
    func loadscroll(){
        ...
        for index in 0..<picArray.count {
            ...
            image.sd_setImageWithURL(NSURL(string:url), completed: { (UIimage:UIImage!, error:NSError!, cacheType:SDImageCacheType, nsurl:NSURL!) -> Void in
                ...
                let photoview = VIPhotoView.init(frame:CGRect(x: CGFloat(index)*self.width,y:-70
...
//    分享功能
    func sharefunc(){
        ...
        let shareimage=imagearray[Int(scrollview.contentOffset.x/width)] as UIImage
...
//        if gestureRec.state == UIGestureRecognizerState.Began {
            ...
            let img = self.imagearray[Int(self.scrollview.contentOffset.x/self.width)] as UIImage
